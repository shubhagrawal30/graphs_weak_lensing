...
class GINE(baseGNN):
    def __init__(self, node_features, edge_features, num_classes, \
        ...
        super(GINE, self).__init__(node_features, edge_features, num_classes, internal_nn_hidden_channels,\
        ...
        self.internal_nn = lambda inp, out: torch.nn.Sequential(\
            Linear(inp, inhc), BatchNorm(inhc), torch.nn.ReLU(), \
                Linear(inhc, out))
...
def set_up_model(dataset, num_classes, device, loss_type="mse"):
    ...
    model = GINE(dataset.num_features, dataset.num_edge_features, num_classes, \
                        num_layers=4, dropout=0.1, negative_slope=0.2, \
